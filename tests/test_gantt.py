...
def test_gantt_chart(mock_show):
    ...
    assert len(ax.patches) == len(categories), (
        "Incorrect number of tasks (bars) created")
    ...
    for p, start_date in zip(ax.patches, start_dates):
        bar_start = mdates.num2date(p.get_x()).replace(tzinfo=None)
        assert bar_start == start_date, (
            "Bar does not start at the correct date:" +
            f" expected {start_date}, got {bar_start}")
